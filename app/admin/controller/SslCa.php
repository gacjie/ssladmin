...
class SslCa extends AdminBase
{

    public function initialize(){
		parent::initialize();
        $this->model = new \app\admin\model\SslCa();
    }
   ...
   public function add(){
	   	if($this->request->isPost()){
            ...
            $SslCaId = Db::name('ssl_ca')->strict(false)->insertGetId($data);
	   		if($SslCaId){
	   		    if($data['ssl_ca_id'] == '0'){
	   		        Db::name('ssl_ca')->update(['ssl_ca_id' => $SslCaId,'id' => $SslCaId]);
	   		    }
...
$insert_data[$k]['switch'] = isset($v[4]) ? $v[4] : '';
}
            }
   ...
   public function sw(){
    ...
    public function down(){
      	$data = $this->request->param();
      	    $set=config('site');
      	    //var_dump($data);exit;
      	    $path=root_path().$set['document'].'/authority/'.md5($data['name']);
